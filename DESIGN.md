...
Pool worker threads and the uv loop thread call `GC_register_my_thread` on start (passing the approximate stack base) and `GC_unregister_my_thread` before exit. This is required so the GC can scan every thread's stack during stop-the-world collection.
...
| `chan_take_from_putter` | `static inline int chan_take_from_putter(goc_chan*, void**)` | `internal.h` — requires `struct goc_chan` in scope; pulled in via `chan_type.h` |
| `chan_put_to_taker` | `static inline int chan_put_to_taker(goc_chan*, void*)` | `internal.h` — requires `struct goc_chan` in scope; pulled in via `chan_type.h` |
...
4. **`loop.c`** — Call `loop_init()`. Internally this: allocates and initialises `g_loop`; malloc-allocates and initialises both `uv_async_t` handles (`g_wakeup` via atomic release store, `g_shutdown_async`); calls `cb_queue_init()` to allocate the MPSC sentinel node (must precede thread spawn and requires GC already up); then spawns the dedicated loop thread. All shared state the loop thread can reach (`g_live_mutex`, pool registry, async handles, callback queue) is fully initialised at this point. The loop thread calls `GC_register_my_thread(GC_get_stack_base())` on entry and `GC_unregister_my_thread()` before exit; it runs `while (uv_run(g_loop, UV_RUN_ONCE)) {}` for its entire lifetime.
