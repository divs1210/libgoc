...
- **Synchronisation**: a `done_t` helper lets the main thread block until fibers finish without relying on `sleep` or a `goc_chan`. In phases P1–P7, `done_t` wraps a POSIX `sem_t` (`done_signal` calls `sem_post`, `done_wait` calls `sem_wait`). Phase P9 uses a portable variant backed by `pthread_mutex_t` + `pthread_cond_t` that also provides a timed wait (`done_wait_ms`), because `sem_timedwait` is not available on macOS.
