...
int post_on_handle_loop(uv_loop_t* loop, void (*fn)(void*), void* arg) {
    GOC_DBG(
            "post_on_handle_loop: entry loop=%p fn=%p arg=%p registry_len=%zu\n",
            (void*)loop, (void*)fn, arg, g_pool_registry_len);
    uv_mutex_lock(&g_pool_registry_mutex);
    for (size_t p = 0; p < g_pool_registry_len; p++) {
        goc_pool* pool = g_pool_registry[p];
        for (size_t i = 0; i < pool->thread_count; i++) {
            if (&pool->workers[i].loop == loop) {
                ...
                GOC_DBG(
                        ...
                        i, (void*)pool, (void*)loop, (void*)w->wakeup, (void*)fn,
                        arg, atomic_load_explicit(&pool->shutdown, memory_order_relaxed),
                        loop_alive);
                ...
                uv_mutex_unlock(&g_pool_registry_mutex);
                atomic_fetch_add_explicit(&w->pending_handle_tasks, 1,
                                          memory_order_acq_rel);
    ...
    GOC_DBG(
            "post_on_handle_loop: loop %p not found in registry, dumping registry contents len=%zu\n",
            (void*)loop, g_pool_registry_len);
    for (size_t p = 0; p < g_pool_registry_len; p++) {
        goc_pool* pool = g_pool_registry[p];
        GOC_DBG("post_on_handle_loop: registry pool=%p thread_count=%zu\n",
                (void*)pool, pool->thread_count);
        for (size_t i = 0; i < pool->thread_count; i++) {
            GOC_DBG(
                    "post_on_handle_loop:   pool=%p worker=%zu loop=%p wakeup=%p\n",
                    (void*)pool, i, (void*)&pool->workers[i].loop,
...
run:
        ...
        /* Drain any I/O callbacks queued on the worker's own loop.  Returns
         * immediately (epoll_wait timeout=0) if nothing is pending. */
        GOC_DBG(
                "pool_worker_fn: worker=%zu begin uv_run(NOWAIT) loop=%p\n",
                tl_worker->index, (void*)&tl_worker->loop);
...
void goc_pool_destroy(goc_pool* pool) {
    ...
    GOC_DBG("goc_pool_destroy: joining %zu workers pool=%p id=%d\n",
    ...
    GOC_DBG("goc_pool_destroy: all workers joined, proceeding to per-worker resource cleanup\n");
