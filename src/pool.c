...
void pool_submit_spawn(goc_pool* pool,
    ...
    if (bypass_throttle ||
        ...
        fprintf(stderr, "[GOC_DBG] pool_submit_spawn: fn=%p bypass=%d resident=%zu live=%zu (fast path)\n",
                (void*)(uintptr_t)fn, (int)bypass_throttle, pool->resident_count, live); fflush(stderr);
    ...
    fprintf(stderr, "[GOC_DBG] pool_submit_spawn: fn=%p bypass=%d DEFERRED (cap reached or queue non-empty)\n",
            (void*)(uintptr_t)fn, (int)bypass_throttle); fflush(stderr);
...
run:
        ...
        fprintf(stderr, "[GOC_DBG] pool_worker_fn: worker=%zu RUNNING fn=%p entry=%p\n",
                tl_worker->index, (void*)(uintptr_t)entry->fn, (void*)entry); fflush(stderr);
        ...
        if (st == MCO_DEAD) {
            fprintf(stderr, "[GOC_DBG] pool_worker_fn: worker=%zu FIBER DEAD fn=%p live_count BEFORE=%zu\n",
                    tl_worker->index, (void*)(uintptr_t)entry->fn,
                    pool->live_count); fflush(stderr);
            if (fe != NULL) {
...
void post_to_run_queue(goc_pool* pool, goc_entry* entry) {
    ...
    fprintf(stderr, "[GOC_DBG] post_to_run_queue: fn=%p entry=%p path=%s\n",
            (void*)(uintptr_t)entry->fn, (void*)entry,
            (w != NULL && w->pool == pool) ? "internal" : "external"); fflush(stderr);
...
void goc_pool_destroy(goc_pool* pool) {
    ...
    uv_mutex_lock(&pool->drain_mutex);
    fprintf(stderr, "[GOC_DBG] goc_pool_destroy: waiting for live_count=%zu\n", pool->live_count); fflush(stderr);
    while (pool->live_count > 0) {
        uv_cond_wait(&pool->drain_cond, &pool->drain_mutex);
        fprintf(stderr, "[GOC_DBG] goc_pool_destroy: cond_wait returned live_count=%zu\n", pool->live_count); fflush(stderr);
    }
    uv_mutex_unlock(&pool->drain_mutex);

    /* 2. Signal workers to exit. */
    fprintf(stderr, "[GOC_DBG] goc_pool_destroy: signaling workers shutdown\n"); fflush(stderr);
    ...
    for (size_t i = 0; i < pool->thread_count; i++) {
        fprintf(stderr, "[GOC_DBG] goc_pool_destroy: joining worker=%zu\n", i); fflush(stderr);
        goc_thread_join(&pool->workers[i].thread);
        fprintf(stderr, "[GOC_DBG] goc_pool_destroy: joined worker=%zu\n", i); fflush(stderr);
    }
