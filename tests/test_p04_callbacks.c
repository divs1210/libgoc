...
static void sender_fiber_fn(void* arg) {
    ...
    goc_put(a->ch, goc_box(unsigned int, a->payload));
}
...
static void test_p4_1(void) {
    ...
    ASSERT(goc_unbox(unsigned int, result.val) == 42);
...
static void test_p4_2(void) {
    ...
    take_cb_result_t result = { .val = goc_box(unsigned int, 0xFF), .ok = GOC_OK, .done = &done };
...
static void taker_fiber_fn(void* arg) {
    ...
    if (v->ok == GOC_OK) {
        a->out = goc_unbox(unsigned int, v->val);
    }
    done_signal(a->done);
}
...
static void test_p4_3(void) {
    ...
    goc_put_cb(ch, goc_box(unsigned int, 99), put_cb, &presult);
...
static void test_p4_5(void) {
    ...
    goc_put_cb(ch, goc_box(unsigned int, 55), put_cb, &result);
...
static void test_p4_6(void) {
    ...
    goc_put_cb(ch, goc_box(unsigned int, 123), put_cb, &result);
    ...
    goc_val_t* v = goc_take_sync(ch);
    ASSERT(v->ok == GOC_OK);
    ASSERT(goc_unbox(unsigned int, v->val) == 123);
...
static void test_p4_7(void) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        put_cb_result_t result = { .ok = GOC_EMPTY, .done = &done };
        goc_put_cb(ch, goc_box(unsigned int, 77), put_cb, &result);
        goc_close(ch);
