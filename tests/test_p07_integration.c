 ...
 *
 * The producer sends integers 1..N as goc_box(unsigned int, ) values.  The
...
static void test_p7_1_producer_fn(void* arg) {
    ...
    for (int i = 1; i <= a->n; i++) {
        goc_status_t st = goc_put(a->src_ch, goc_box(unsigned int, i));
        if (st != GOC_OK) break;
    }
    goc_close(a->src_ch);
}
...
static void test_p7_1_transformer_fn(void* arg) {
    ...
    for (;;) {
        ...
        size_t* val = v->val;
        *val = *val * 2;
        goc_status_t st = goc_put(a->dst_ch, val);
        if (st != GOC_OK) break;
    }
    goc_close(a->dst_ch);
}
...
static void test_p7_1_consumer_fn(void* arg) {
    ...
    for (;;) {
        ...
        if (v->ok != GOC_OK) break;
        sum += goc_unbox(unsigned int, v->val);
    }
    a->sum = sum;
    done_signal(a->done);
}
...
static void test_p7_2_producer_fn(void* arg) {
    ...
    for (int i = 1; i <= a->n; i++) {
        goc_status_t st = goc_put(a->work_ch, goc_box(unsigned int, i));
        if (st != GOC_OK) break;
    }
    goc_close(a->work_ch);
}
...
static void test_p7_2_worker_fn(void* arg) {
    ...
    for (;;) {
        ...
        if (v->ok != GOC_OK) break;
        size_t result = goc_unbox(size_t, v->val) * 2;
        goc_put(a->result_ch, goc_box(size_t, result));
    }
...
static void test_p7_3_producer_fn(void* arg) {
    ...
    for (int i = 1; i <= a->n; i++) {
        goc_status_t st = goc_put(a->ch, goc_box(unsigned int, i));
        if (st != GOC_OK) break;
    }
    goc_close(a->ch);
}
...
static void test_p7_4_sender_fn(void* arg) {
    ...
    goc_put(a->ch, goc_box(unsigned int, a->id));
}
...
static void test_p7_4(void) {
    ...
    for (int i = 0; i < P7_4_NSENDERS; i++) {
        goc_val_t* v = goc_take_sync(ch);
        ASSERT(v->ok == GOC_OK);
        uintptr_t id = goc_unbox(unsigned int, v->val);
...
static void test_p7_5_worker_fn(void* arg) {
    ...
    goc_put(a->result_ch, goc_box(unsigned int, a->value));
}
...
static void fast_fiber_wrapper(void* arg) {
    ...
    goc_put(ffa->args->result_ch, goc_box(unsigned int, ffa->args->value));
}
...
static void test_p7_5(void) {
    ...
    ASSERT(goc_unbox(unsigned int, r->value.val) == 0xFADE);
