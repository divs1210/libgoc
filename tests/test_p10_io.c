 ...
 * Test coverage:
 *
 *   P10.1  goc_fs_open_ch: open a new file; result file descriptor >= 0
 *   P10.2  goc_fs_write_ch: write data to an open file; result == written bytes
 *   P10.3  goc_fs_read_ch: read back the data; matches written content
 *   P10.4  goc_fs_stat_ch: stat the file; size and type fields correct
 *   P10.5  goc_fs_rename_ch: rename the file; stat old path fails, new path ok
 *   P10.6  goc_fs_unlink_ch: delete the file; subsequent stat fails
 *   P10.7  goc_fs_open + goc_fs_close blocking wrappers (fiber context)
 *   P10.8  goc_fs_open_ch with invalid path: result < 0 (error code)
 *   P10.9  goc_getaddrinfo_ch: resolve "localhost"; status == 0, res != NULL
 *   P10.10 goc_getaddrinfo_ch with empty node and service: returns error
 *   P10.11 goc_write_ch / goc_write2_ch / goc_shutdown_stream_ch channel
 *          variants compile and return non-NULL channels (no network I/O)
 *   P10.12 goc_fs_sendfile_ch: copy bytes between two file descriptors
 *   P10.13 Channel-based goc_fs_open_ch integrates with goc_alts (select
...
{
    TEST_BEGIN("P10.1  goc_fs_open_ch opens a new file");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.2  goc_fs_write_ch: write data
 * ====================================================================== */
...
{
    TEST_BEGIN("P10.2  goc_fs_write_ch writes correct byte count");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.3  goc_fs_read_ch: read back written content
 * ====================================================================== */

static void fiber_p10_3(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_fs_open_t* opened = goc_fs_open(TMP_PATH, O_RDONLY, 0);
    if (!opened || opened->result < 0) goto done;
    uv_file fd = opened->result;

    char readbuf[64] = {0};
    uv_buf_t buf = uv_buf_init(readbuf, sizeof(readbuf) - 1);
    goc_fs_read_t* rd = goc_fs_read(fd, &buf, 1, 0);
...
{
    TEST_BEGIN("P10.3  goc_fs_read_ch reads back correct content");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.4  goc_fs_stat_ch: stat the file
 * ====================================================================== */

static void fiber_p10_4(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_fs_stat_t* st = goc_fs_stat(TMP_PATH);
...
{
    TEST_BEGIN("P10.4  goc_fs_stat_ch reports correct file size");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.5  goc_fs_rename_ch: rename the file
 * ====================================================================== */

static void fiber_p10_5(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_fs_rename_t* ren = goc_fs_rename(TMP_PATH, TMP_PATH2);
    if (!ren || ren->result != 0) goto done;

    /* Old path should no longer exist */
    goc_fs_stat_t* old_st = goc_fs_stat(TMP_PATH);
    if (!old_st || old_st->result == 0) goto done;  /* still exists = fail */

    /* New path should exist with the same size */
    goc_fs_stat_t* new_st = goc_fs_stat(TMP_PATH2);
...
{
    TEST_BEGIN("P10.5  goc_fs_rename_ch renames file correctly");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.6  goc_fs_unlink_ch: delete the (renamed) file
 * ====================================================================== */

static void fiber_p10_6(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_fs_unlink_t* ul = goc_fs_unlink(TMP_PATH2);
    if (!ul || ul->result != 0) goto done;

    /* File should no longer exist */
    goc_fs_stat_t* st = goc_fs_stat(TMP_PATH2);
...
{
    TEST_BEGIN("P10.6  goc_fs_unlink_ch deletes file, stat then fails");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.7  Blocking wrappers (goc_fs_open + goc_fs_close) in fiber context
 * ====================================================================== */
...
static void fiber_p10_7(void* arg)
{
    ...
    goc_fs_open_t* opened = goc_fs_open(TMP_PATH,
                                         O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (!opened || opened->result < 0) goto done;
    goc_fs_close_t* closed = goc_fs_close((uv_file)opened->result);
...
{
    TEST_BEGIN("P10.7  goc_fs_open + goc_fs_close blocking wrappers work");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.8  goc_fs_open_ch with non-existent path + O_RDONLY → error
 * ====================================================================== */

static void fiber_p10_8(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_chan* ch = goc_fs_open_ch("/nonexistent/path/that/does/not/exist",
                                   O_RDONLY, 0);
    goc_val_t*     v  = goc_take(ch);
    if (!v || v->ok != GOC_OK) goto done;
    goc_fs_open_t* res = (goc_fs_open_t*)v->val;
...
{
    TEST_BEGIN("P10.8  goc_fs_open_ch with invalid path returns error code");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.9  goc_getaddrinfo_ch: resolve "localhost"
 * ====================================================================== */

static void fiber_p10_9(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_chan*           ch = goc_getaddrinfo_ch("localhost", NULL, NULL);
    goc_val_t*          v  = goc_take(ch);
    if (!v || v->ok != GOC_OK) goto done;
    goc_getaddrinfo_t*  res = (goc_getaddrinfo_t*)v->val;
...
{
    TEST_BEGIN("P10.9  goc_getaddrinfo_ch resolves \"localhost\"");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.10 goc_getaddrinfo_ch with node=NULL and service=NULL → error
 * ====================================================================== */

static void fiber_p10_10(void* arg)
{
    fiber_result_t* r = (fiber_result_t*)arg;
    goc_chan*          ch  = goc_getaddrinfo_ch(NULL, NULL, NULL);
    goc_val_t*         v   = goc_take(ch);
    if (!v || v->ok != GOC_OK) goto done;
    goc_getaddrinfo_t* res = (goc_getaddrinfo_t*)v->val;
    if (!res) goto done;
...
{
    TEST_BEGIN("P10.10 goc_getaddrinfo_ch NULL node+service: no crash");
    fiber_result_t r = {0};
...
{
    TEST_BEGIN("P10.11 goc_getaddrinfo_ch returns non-NULL channel");
    goc_chan* ch = goc_getaddrinfo_ch("localhost", NULL, NULL);
    ASSERT(ch != NULL);
...
static void fiber_p10_12(void* arg)
{
    ...
    goc_fs_open_t* src_open = goc_fs_open(TMP_PATH,
                                            O_WRONLY | O_CREAT | O_TRUNC, 0644);
    ...
    goc_fs_write_t* written = goc_fs_write(src_fd, &wbuf, 1, 0);
    goc_fs_close(src_fd);
    if (!written || written->result != CONTENT_LEN) goto done;

    /* Reopen source for reading */
    src_open = goc_fs_open(TMP_PATH, O_RDONLY, 0);
    if (!src_open || src_open->result < 0) goto done;
    src_fd = src_open->result;

    /* Create destination file */
    goc_fs_open_t* dst_open = goc_fs_open(TMP_PATH3,
                                            O_WRONLY | O_CREAT | O_TRUNC, 0644);
    ...
    goc_fs_sendfile_t* sf = goc_fs_sendfile(dst_fd, src_fd, 0,
                                             (size_t)CONTENT_LEN);
    goc_fs_close(src_fd);
    goc_fs_close(dst_fd);
    ...
    goc_fs_open_t* verify_open = goc_fs_open(TMP_PATH3, O_RDONLY, 0);
    if (!verify_open || verify_open->result < 0) goto done;
    char rbuf[64] = {0};
    uv_buf_t rbufv = uv_buf_init(rbuf, sizeof(rbuf) - 1);
    goc_fs_read_t* rd = goc_fs_read(verify_open->result, &rbufv, 1, 0);
    goc_fs_close(verify_open->result);
    if (!rd || rd->result != CONTENT_LEN) goto done;
...
{
    TEST_BEGIN("P10.12 goc_fs_sendfile_ch copies correct byte count");
    fiber_result_t r = {0};
...
/* =========================================================================
 * P10.13 goc_fs_open_ch integrates with goc_alts (select on two I/O ops)
 * ====================================================================== */
...
static void fiber_p10_13(void* arg)
{
    ...
    goc_chan* open_ch  = goc_fs_open_ch(TMP_PATH,
                                         O_WRONLY | O_CREAT | O_TRUNC, 0644);
    ...
    goc_fs_open_t* res = (goc_fs_open_t*)result->value.val;
    if (!res || res->result < 0) goto done;
...
{
    TEST_BEGIN("P10.13 goc_fs_open_ch works with goc_alts (select)");
    fiber_result_t r = {0};
